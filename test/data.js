// Aqui puedes agregar los datos que necesites para tus pruebas
// al menos debe tener uno que se llame "data"

export const data =[
  { num: "001", name: "bulbasaur", 
    type: ["grass", "poison"],basePromedio:119 },
  { num: "002", name: "ivysaur", 
    type: ["grass","poison"],basePromedio: 149.66666666666666},
  { num: "003", name: "venusaur", 
    type: ["grass", "poison"],basePromedio:192.33333333333334 }
];

export const salidaTipo = [
  { num: "001", name: "bulbasaur",
    type: ["grass", "poison"],basePromedio:119 },

  { num: "003", name: "venusaur", 
    type: ["grass", "poison"], basePromedio:192.33333333333334},
];


// export const data = [
//   {
//     "num": "001",
//     "name": "bulbasaur",
//     "type": [
//       "grass",
//       "poison"
//     ],
//     "stats": {
//       "base-attack": "118",
//       "base-defense": "111",
//       "base-stamina": "128",
//     },
//   },

//   {
//     "num": "002",
//     "name": "ivysaur",
//     "type": [
//       "grass",
//       "poison"
//     ],
//     "stats": {
//       "base-attack": "151",
//       "base-defense": "143",
//       "base-stamina": "155",
//     },
//   },
//   {
//     "num": "003",
//     "name": "venusaur",
//     "type": [
//       "grass",
//       "poison"
//     ],
//     "stats": {
//       "base-attack": "198",
//       "base-defense": "189",
//       "base-stamina": "190",
//     },
//   }

// ];
